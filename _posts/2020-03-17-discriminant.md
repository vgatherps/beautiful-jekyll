---
layout: post
title: Using SIMD to Rapidly Distinguish Between Variable-Layout Messages
---

Some years ago, I wrote a nifty SIMD (single-instruction multiple-data) scanner to quickly discover "interesting" fields in a variable layout message format so they could immediately be handled before full processing occurred.

Thinking back, I realized most of these messages probably came in some subset of fixed layouts, so it should be possible to check for a given layout by examining a small subset of bytes.
Using the power of SIMD, it should even be possible to do this in a single pass by checking every possible layout at once.

### Benchmarks preview
Since everybody loves benchmarks, here's a preview of the final results (in cycles, without accounting for timestamping cost):


|Test|Cached|Uncached|
|----|------|--------|
|Discriminant|42|340|
|Discriminant w/o length checks| 39 | 340|
|Scanning|2900|3900|
...
#### A fast scanning parser

The scanning algorithm I previously wrote essentially did the following:

1. Check 16 bytes of the message to see if they equal a begin-message header.
2. Convert this into a bitmask.
3. For each discovered header in the message.
  * Check if the header is considered interesting.
  * If so, save the message + tag into the output
4. Repeat step 1 on the next 16 bytes until there's no more data.
5. Parse the found fields of out the message.

The actual code to do this can be found at (LINK):
...
To see the performance, I benchmark six cases against a 729-byte message:

1. The discriminant
2. The discriminant without length checks
3. SIMD-scanning the message
...
The results, in cycles, and without removing the minor cost of time management, are:
...
The results are pretty expected - the scanner results aren't far from what I measured in production, and the discriminant costs are what I would expect from the generated code. Further, doing branchless length checks has just a minor cost.

An interesting note is that the scanner is much more affected by cache latency than the discriminant. This result, while expected, is worth diving into.
...
Effectively, it only sees the cost of one cache miss. The scanner has to load the entire message and does so with complex control flow.
Outside of the prefetcher, the scanner must process earlier portions of the message before advancing far and loading data.

Overall, the discriminant handily trounces the scanning method, even though a single static message is the best case for the scanner.
Given the branchless nature of the discriminant method, it is expected to also have much more reliable latency distributions.
In a followup post, I'll dive into these measurements as well as measure the cost of branching vs branchless on variable messages.
